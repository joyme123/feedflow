...
export function listSources(): Source[] {
  ...
  const rows = db.prepare(`
    SELECT id, plugin_id as pluginId, name, config, enabled, sort_order as sortOrder,
           cursor_value as cursorValue, created_at as createdAt, updated_at as updatedAt
...
export function addSource(input: AddSourceInput): Source {
  ...
  db.prepare(`
    INSERT INTO sources (id, plugin_id, name, config, sort_order, created_at, updated_at)
    VALUES (?, ?, ?, ?, (SELECT COALESCE(MAX(sort_order), -1) + 1 FROM sources), ?, ?)
  `).run(id, input.pluginId, input.name, JSON.stringify(input.config), now, now)
